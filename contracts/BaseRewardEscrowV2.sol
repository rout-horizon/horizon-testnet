...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    /**
     * @notice Create an escrow entry to lock HZN for a given duration in seconds
     ...
     * @param duration The duration that HZN will be emitted.
     */
    ...
    function nominateAccountToMerge(address account) external {
        ...
        require(issuer().debtBalanceOf(msg.sender, "zUSD") == 0, "Cannot merge accounts with debt");
    ...
    function mergeAccount(address from, uint256[] calldata entryIDs) external {
        require(accountMergingIsOpen(), "Account merging has ended");
        require(issuer().debtBalanceOf(from, "zUSD") == 0, "Cannot merge accounts with debt");
