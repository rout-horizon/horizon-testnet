...
contract EtherWrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IEtherWrapper {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[0] = CONTRACT_ZASSETZBNB;
        newAddresses[1] = CONTRACT_ZASSETZUSD;
    ...
    function totalIssuedSynths() public view returns (uint) {
        ...
        return exchangeRates().effectiveValue(zBNB, sETHIssued, zUSD).add(sUSDIssued);
    }
    ...
    function distributeFees() external {
        ...
        require(!exchangeRates().rateIsInvalid(zBNB), "Currency rate is invalid");
        uint amountSUSD = exchangeRates().effectiveValue(zBNB, feesEscrowed, zUSD);
        ...
        issuer().synths(zUSD).issue(feePool().FEE_ADDRESS(), amountSUSD);
