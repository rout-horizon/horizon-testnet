...
contract FuturesMarketSettings is Owned, MixinFuturesMarketSettings, IFuturesMarketSettings {
    ...
    function setMinKeeperFee(uint _zUSD) external onlyOwner {
        require(_zUSD <= _minInitialMargin(), "min margin < liquidation fee");
        _flexibleStorage().setUIntValue(SETTING_CONTRACT_NAME, SETTING_MIN_KEEPER_FEE, _zUSD);
        emit MinKeeperFeeUpdated(_zUSD);
    }
    ...
    event MinKeeperFeeUpdated(uint zUSD);
