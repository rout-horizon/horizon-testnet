...
const purgeSynths = async ({
	...
	if (synthsToPurge.length < 1) {
		console.log(gray('No zassets provided. Please use --synths-to-purge option'));
		return;
	}
		...
		try {
			await confirmAction(
				cyan(
					`${yellow(
						'⚠ WARNING'
					)}: This action will purge the following zassets from the Horizon contract on ${network}:\n- ${synthsToPurge.join(
						'\n- '
					)}`
