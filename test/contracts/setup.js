...
const setupContract = async ({
	...
	const defaultArgs = {
		...
		FuturesMarketBTC: [
			tryGetAddressOf('AddressResolver'),
			toBytes32('zBTC'), // base asset
			toBytes32('zBTC' + perpSuffix), // market key
		],
		FuturesMarketETH: [
			tryGetAddressOf('AddressResolver'),
			toBytes32('zBNB'), // base asset
			toBytes32('zBNB' + perpSuffix), // market key
		],
...
const setupAllContracts = async ({
	...
	if (returnObj['ExchangeRates']) {
		...
		const keys = ['HZN', ...(feeds || [])].map(toBytes32);
